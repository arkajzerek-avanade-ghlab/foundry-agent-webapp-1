...
| `AI_AGENT_IDS` | .env | Comma-separated agent names for multi-agent mode (including prompt agents) | `agent-a,agent-b` |
...
For multi-agent configuration, create agents first and set `AI_AGENT_IDS`. Leave `AI_AGENT_ID` empty to avoid forcing single-agent behavior.
