...
| `AI_AGENT_IDS` | .env | Comma-separated agent names (single or multiple); first entry is the default | `agent-a,agent-b` |
...
For agent configuration, set `AI_AGENT_IDS` to a comma-separated list of agent names. The first entry is used as the default agent.
